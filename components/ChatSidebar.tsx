...
export const ChatSidebar = ({ selectedAssistant, onAssistantChange, onNewChat, onSessionSelect, refreshTrigger }: ChatSidebarProps) => {
  ...
  return (
    <div className={`w-64 md:w-80 lg:w-80 bg-[#0a1628] ${isRTL ? 'border-l' : 'border-r'} border-[#1a2332] flex flex-col h-screen relative`}>
      {/* Fixed Top Header (ChatGPT-style) */}
      <div className="px-4 py-2 border-b border-[#1a2332] bg-[#0a1628] flex-shrink-0 relative z-30">
        <div className="relative overflow-hidden min-h-[104px] mb-1.5">
          <div
            className={`flex items-center ${
              isRTL
                ? 'mr-4 md:mr-8 -mt-1 md:-mt-2 pl-14'
                : 'ml-4 md:ml-8 -mt-1 md:-mt-2 pr-14'
            }`}
          >
          ...
          <button
            ...
            className={[
              ...
              'flex items-center gap-1.5 px-2.5 py-1.5 rounded-full border border-[#1a2332] bg-[#0a1628]/40 text-gray-200 hover:text-white hover:bg-[#1a2332]/40 transition-all duration-200 text-[11px] whitespace-nowrap',
            ].join(' ')}
          >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain [scrollbar-gutter:stable]">
        ...
        <div className="px-3 py-2 border-b border-[#1a2332]">
          <button
            ...
            className={`w-full flex items-center gap-2 px-1 py-2 text-red-400 hover:text-red-300 hover:bg-red-900/10 transition-colors rounded-md ${
              isRTL ? 'flex-row-reverse text-right' : 'text-left'
            }`}
          >
            <MessageSquare className="h-4 w-4 flex-shrink-0" />
            <span className="text-[13px] font-medium">{t('common.newChat')}</span>
          </button>

          <button
            type="button"
            onClick={() => setSearchOpen((v) => !v)}
            className={`w-full flex items-center gap-2 px-1 py-2 text-gray-200 hover:text-white hover:bg-white/5 transition-colors rounded-md ${
              isRTL ? 'flex-row-reverse text-right' : 'text-left'
            }`}
          >
            <Search className="h-4 w-4 flex-shrink-0" />
            <span className="text-[13px] font-medium">{t('common.searchChats')}</span>
          </button>
        ...
        {/* Assistant Selection */}
        <div className="px-3 py-2 border-b border-[#1a2332] bg-[#0a1628]">
          <div className="relative">
            <button
              type="button"
            onClick={() => setIsAssistantDropdownOpen(!isAssistantDropdownOpen)}
              className={`w-full flex items-center justify-between gap-3 px-1 py-2 text-gray-200 hover:text-white hover:bg-white/5 transition-colors rounded-md ${
                isRTL ? 'flex-row-reverse text-right' : 'text-left'
              }`}
          >
              <span className="flex items-center gap-2 min-w-0">
                <MessageSquare className="h-4 w-4 flex-shrink-0" />
                <span className="text-[13px] font-medium truncate">{getAssistantName(selectedAssistant)}</span>
            </span>
              <ChevronDown className={`h-4 w-4 flex-shrink-0 transition-transform duration-200 ${isAssistantDropdownOpen ? 'rotate-180' : ''}`} />
            </button>
      ...
      {/* My Courses Section */}
        <div className="border-b border-[#1a2332] px-3 py-1">
        <button
            type="button"
          onClick={() => setCoursesExpanded(!coursesExpanded)}
            className={`w-full flex items-center justify-between gap-3 px-1 py-2 text-gray-300 hover:text-white hover:bg-white/5 transition-colors rounded-md ${
              isRTL ? 'flex-row-reverse text-right' : 'text-left'
            }`}
        >
                ...
                {courses.map((course) => (
                  <button
                    ...
                    onClick={() => handleCourseClick(course.course_id)}
                      className={`w-full group flex items-center gap-3 p-2.5 rounded-lg hover:bg-[#1a2332]/60 transition-all duration-150 ${
                        isRTL ? 'flex-row-reverse text-right' : 'text-left'
                      }`}
                  >
                      <GraduationCap className="h-4 w-4 text-[#dc2626] flex-shrink-0" />
        ...
        <div className="px-3 py-1">
          <div className={`w-full flex items-center justify-between gap-3 px-1 py-2 text-gray-300 ${
            ...
            <span className="text-[13px] font-medium">{t('chatHistory.recentChats')}</span>
          </div>
        </div>
              ...
              {filteredSessions.map((session) => (
                <button
                  ...
                  >
                    ...
                    <MessageSquare className="h-4 w-4 text-[#60a5fa] flex-shrink-0" />
                      )}
      ...
      {/* Footer */}
      <div className="px-3 py-3 border-t border-[#1a2332] bg-[#0a1628] flex-shrink-0">
        <div className={`flex gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
          <Button
            variant="ghost"
            onClick={() => router.push('/settings')}
            className="flex-1 text-gray-400 hover:text-white hover:bg-[#1a2332]/60 h-10 text-sm rounded-lg transition-all duration-150 justify-center"
          >
          ...
          <Button
            ...
            className="flex-1 text-red-400 hover:text-red-300 hover:bg-red-900/10 h-10 text-sm rounded-lg transition-all duration-150 justify-center"
          >
